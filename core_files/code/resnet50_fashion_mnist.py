...
callbacks = [
    ...
    ModelCheckpoint('resnet_best_fashion_mnist_model.keras', monitor='val_accuracy', save_best_only=True),
...
cnn_mnist.fit(X_train, y_train, epochs=100, batch_size=32, validation_split=0.2, callbacks=callbacks, verbose=1)
...
cnn_fft.fit(X_train_fft, y_train, epochs=100, batch_size=32, validation_split=0.2, callbacks=callbacks, verbose=1)
...
cnn_wavelet.fit(X_train_wavelet, y_train, epochs=100, batch_size=32, validation_split=0.2, callbacks=callbacks, verbose=1)
...
# Output directories
os.makedirs("simple_resnet_fashion_mnist_confusion_matrices", exist_ok=True)
os.makedirs("simple_resnet_fashion_mnist_correlation_plots", exist_ok=True)
os.makedirs("simple_resnet_fashion_mnist_predition_output", exist_ok=True)
...
for p in mask_percentages:
    ...
    def evaluate_and_save(model, X_test_transformed, name, p):
        ...
        df_cm.to_csv(f"simple_resnet_fashion_mnist_confusion_matrices/{name}_confusion_{p}.csv")
        df_prediction = pd.DataFrame()
        df_prediction['True Label'] = y_test
        df_prediction['Predicted Label'] = y_pred
        df_prediction.to_csv(f"simple_resnet_fashion_mnist_predition_output/{name}_predictions_{p}.csv")
    ...
    plt.savefig(f"simple_resnet_fashion_mnist_correlation_plots/correlation_plot_{p}.png")
    plt.close()

df_accuracy = pd.DataFrame(accuracy_results, index=range(0, 100, 1))
df_accuracy.to_csv("simple_resnet_fashion_mnist_predition_output/accuracy_results.csv", index_label="Data Loss %")
...
plt.ylim((0,1.1))
# Save the plot
plt.savefig("simple_resnet_fashion_mnist_correlation_plots/Accuracy_vs_DataLoss.png")
plt.show()
...
plt.legend()
plt.grid(True)
plt.savefig("resnet_fashion_mnist_ADR_vs_DataLoss.png")
plt.show()
